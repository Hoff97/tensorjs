...
export class AddNode extends OnnxNode {
  ...
  async forward(inputs: Tensor[]): Promise<Tensor[]> {
    if (this.onnxVersion < 13 && this.onnxVersion >= 7) {
      ...
      if (!this.compiled) {
        ...
        return [this.addOperation.calc({A: _a as GPUTensor, B: _b as GPUTensor, outputShape: resultShape as number[]})];
      }
    }
  ...
  async staticForward(inputs: Tensor[], compile?: boolean): Promise<{ outputs: (CPUTensor | PrototypeTensor)[]}> {
    ...
    if (this.onnxVersion < 13 && this.onnxVersion >= 7) {
      ...
      if (compile) {
        ...
        this.addOperation.compile(info);
