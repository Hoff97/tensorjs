...
export interface ArgMinInput {
  X: GPUTensorI;
  axes: number[];
  selectLast: boolean;
}

export class ArgMinOperation<GPUTensor extends GPUTensorI> extends Operation<
  GPUTensor,
  ArgMinInfo,
  ArgMinInput
> {
  ...
  getFragmentShader(info: ArgMinInfo): string {
    return `
  ...
  calc(input: ArgMinInput): GPUTensor {
  ...
  getOutputShape(input: ArgMinInput): readonly number[] {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  getCompilationInfo(input: ArgMinInput): ArgMinInfo {
    // eslint-disable-next-line prefer-const
    let [outputShape, ixMap] = poolResultShape(
      input.X.shape,
      input.axes,
  ...
  getInputInfoString(input: ArgMinInput): string {
