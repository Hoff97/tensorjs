...
export function reduceLogSumExpSparseWASM<DTpe extends DTypeWasm>(
  ...
  if (tensor.dtype !== 'float32' && tensor.dtype === 'float64') {
    throw new Error(
      'Reduce log sum exp expects tensor datatype to be float, but found ' +
        tensor.dtype
    );
  }
  return new WASMTensor(
    ((tensor.values as WASMTensor<DTpe>)
      .wasmTensor as any).reduce_log_sum_exp_sparse(
