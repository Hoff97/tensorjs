...
export function subtractDenseWASM<DTpe extends DTypeWasm>(
  a: SparseTensor<DTpe>,
  b: WASMTensor<DTpe>,
  ...
  const vals = new WASMTensor(
    (a.values as WASMTensor<DTpe>).wasmTensor.subtract_sparse_dense(
...
export function subtractSparseWASM<DTpe extends DTypeWasm>(
  a: SparseTensor<DTpe>,
  b: SparseTensor<DTpe>,
  resultShape: readonly number[],
  alpha: number,
  ...
  const vals = new WASMTensor(
    (a.values as WASMTensor<DTpe>).wasmTensor.subtract_sparse_sparse(
      (a.indices as WASMTensor<'uint32'>).wasmTensor,
