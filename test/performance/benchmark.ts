...
const cpuConstructor = (shape: ReadonlyArray<number>, values: number[]) =>
...
suite(
  ...
  },
  {
    ...
    onComplete() {
      for (const backend of backends) {
        for (const tensor of (utility as any).tensors[backend.name]) {
          tensor.delete();
        }
      }
      utility = {};
...
suite(
  ...
  () => {
    for (const backend of backends) {
      benchmark(backend.name, () => {
        const result = (utility as any).tensors[backend.name][0].matMul(
          (utility as any).tensors[backend.name][1]
        );
        result.delete();
      });
    }
  ...
  suite(
    ...
    },
    {
      onStart() {
        ...
        const tensors: {[name: string]: Tensor} = {};
        for (const backend of backends) {
          tensors[backend.name] = backend.constructor(
            [size, size, size, size],
            values1
          );
        }
