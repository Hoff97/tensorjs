...
for (const backend of backends) {
  describe(`Sparse tensor on ${backend.name}`, () => {
    ...
    it('should work with log sum exp over dense dimensions', async () => {
      if (backend.wait !== undefined) {
        await backend.wait;
      }
      ...
      const valueValsResult1 = [
        ...
        Math.log(Math.exp(3) + Math.exp(4)),
        Math.log(Math.exp(5) + Math.exp(6)),
        Math.log(Math.exp(7) + Math.exp(8)),
      ];
      const valueTensorResult1 = backend.constructor(
        [4],
        valueValsResult1,
      ...
      const res1 = tensorA.reduceLogSumExp(2) as SparseTensor;
      expect(res1.nnz).toBe(4);
      expect(res1.shape).toEqual([3, 3]);
    ...
    it('should work with log sum exp over sparse dimension 1', async () => {
      if (backend.wait !== undefined) {
        await backend.wait;
      }
      ...
      const result = a.reduceLogSumExp(0);

      const expected = await backend.constructor(
        [4],
        [1, Math.log(Math.exp(3) + Math.exp(4)), 5, 2],
        'float32'
      );
    ...
    it('should work with log sum exp over sparse dimension 2', async () => {
      if (backend.wait !== undefined) {
        await backend.wait;
      }
      ...
      const result = a.reduceLogSumExp(1);
      ...
      const expected = await backend.constructor(
        ...
        [
          Math.log(Math.exp(1) + Math.exp(2)),
          3,
          Math.log(Math.exp(4) + Math.exp(5)),
        ],
        'float32'
      );
